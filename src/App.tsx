...
function Modal({title,onClose,wide,xwide,children}){return<div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"><div className={`bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full ${xwide?"max-w-5xl":wide?"max-w-2xl":"max-w-lg"} max-h-[93vh] overflow-y-auto`}><div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white z-10"><h2 className="font-bold text-gray-800 text-base">{title}</h2><button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-400 text-xl">&times;</button></div><div className="px-5 py-5">{children}</div></div></div>;}
...
const lineas=doc.splitTextToSize(p.desc,p.tipo==="normal"||!p.tipo?118:175);
    ...
    doc.text(`${cant}`,156,y,{align:"right"});
        doc.text(`${prec}€`,174,y,{align:"right"});
...
<FichaTrabajo t={t} cl={cl} co={co} data={data} setData={setData} onClose={onClose} toast={toast} setModo={setModo} setSec={setSec}/>
...
function DisponibilidadSelector({onConfirmar}:{onConfirmar:(dia:string,hora:string,importe:string,archivo:File|null)=>void}){
  ...
  return<div className="space-y-3">
    ...
    <button onClick={()=>{if(dia)onConfirmar(dia,hora,importe,archivo);}} disabled={!dia} className="w-full bg-green-500 hover:bg-green-600 text-white py-3 rounded-xl font-bold text-sm transition disabled:opacity-50">✅ Confirmar</button>
  </div>;
}
