...
function Colaboradores({data,setData,onBack,toast}){
  ...
  if(!coid){
    ...
    return<div>
<Back title="Colaboradores" onBack={onBack} right={<div className="flex gap-2"><button onClick={()=>{const msg=`Hola 👋\n\nTe invitamos a unirte como colaborador de *Domia Services*.\n\nRellena tus datos aquí para darte de alta:\nhttps://domia-crm-two.vercel.app/alta-colaborador\n\n¡Gracias!\n\n— Samuel · Domia Services`;navigator.clipboard.writeText(msg).then(()=>{toast("📋 Mensaje copiado — pégalo en WhatsApp");window.open("https://web.whatsapp.com","_blank");});}} className="bg-green-500 text-white text-xs font-bold px-3 py-1.5 rounded-xl hover:bg-green-600 transition">📤 Enviar alta</button><button onClick={()=>setShowNew(true)} className="bg-[#1E3A5F] text-white text-xs font-bold px-3 py-1.5 rounded-xl hover:bg-[#152d4a] transition">+ Nuevo</button></div>}/>
