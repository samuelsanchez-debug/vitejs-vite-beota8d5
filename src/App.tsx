...
const trabajosConColab=data.trabajos.filter(t=>getColabId(t)&&["Aceptado","En curso","Completado"].includes(t.estado)&&(!focoTrabajo||t.id===focoTrabajo));
const trabajosCliente=data.trabajos.filter(t=>["Aceptado","En curso","Completado"].includes(t.estado)&&(!focoTrabajo||t.id===focoTrabajo));
...
function PortalCliente({id}:{id:string}){
  ...
  if(estado==="no")return<div className="min-h-screen flex items-center justify-center bg-[#F0F2F5] p-4"><div className="bg-white rounded-2xl p-8 text-center max-w-sm w-full shadow-sm border border-gray-100"><div className="text-6xl mb-4">📞</div><div className="text-xl font-black text-gray-800 mb-2">Entendido</div><div className="text-gray-500 text-sm">Te llamaremos para buscar otra fecha que te venga mejor.</div><div className="mt-4 text-xs text-gray-400">Domia Services · 685 917 059</div></div></div>;
  return<div className="min-h-screen bg-[#F0F2F5]" style={{fontFamily:"'Inter',system-ui,sans-serif"}}>
    <div className="bg-[#1E3A5F] px-5 py-6 text-white text-center">
