...
// Helpers para manejar tanto camelCase (local) como snake_case (Supabase)
...
function FormTrabajo({data,setData,inicial,onClose,toast}){
  ...
  const[f,setF]=useState(inicial?{
    ...inicial,
    clienteId: getClienteId(inicial),
    colaboradorId: getColabId(inicial),
    presupuestoColaborador: getPresupColab(inicial),
    precioCliente: getPrecioCliente(inicial),
  }:{
  ...
  const save=async()=>{
    ...
    const historial=esNuevo?[
      {ts:now(),txt:`Solicitud recibida por ${o.label}`,tipo:"entrada"},
      ...(colab?[{ts:now(),txt:`Asignado a ${colab.nombre}`,tipo:"sistema"},{ts:now(),txt:"Presupuesto solicitado por WhatsApp",tipo:"wa"}]:[])
    ]:[...(getHistorial(inicial)),{ts:now(),txt:"Trabajo actualizado",tipo:"sistema"}];
    ...
    const trabajo={
      ...
      estado:esNuevo?(colab?"Presupuestando":"Solicitud"):f.estado,
      historial,
    };
    ...
    if(saved){
      if(esNuevo){
        setData(d=>({...d,trabajos:[...d.trabajos,{...saved,clienteId:saved.cliente_id,colaboradorId:saved.colaborador_id}]}));
        if(colab){toast("✅ Registrado — abriendo WhatsApp");setTimeout(()=>window.open(buildWA(colab,trabajo,cliente),"_blank"),400);}
        else toast("✅ Solicitud registrada");
      } else {
        setData(d=>({...d,trabajos:d.trabajos.map(x=>x.id===f.id?{...saved,clienteId:saved.cliente_id,colaboradorId:saved.colaborador_id}:x)}));
        toast("✅ Guardado");
      }
    }
    onClose();
...
function Home({data,setData,go,setTid,toast}){
  ...
  return<div className="space-y-5">
    ...
    <div className="grid grid-cols-2 gap-3">
      {[{s:"nuevas",icon:"📥",label:"Nuevas demandas",desc:"Sin gestionar",badge:nuevas,bc:"bg-red-500"},{s:"demandas",icon:"📋",label:"Estado demandas",desc:"Pipeline completo",badge:activos,bc:"bg-orange-500"},{s:"clientes",icon:"👥",label:"Clientes",desc:"Base de datos",badge:0,bc:""},{s:"colaboradores",icon:"👷",label:"Colaboradores",desc:"Equipo y filtros",badge:0,bc:""}].map(m=>(
    ...
    {nuevas>0&&<div className="bg-red-50 border border-red-200 rounded-2xl p-4">
      ...
      {sinAsignar.slice(0,2).map(t=>{
        ...
        return<div key={t.id} className="flex items-center justify-between gap-2 mt-2 bg-white rounded-xl px-3 py-2 border border-red-100">
          ...
          {sg&&<button onClick={async()=>{
            const waUrl=buildWA(sg,t,cl);
            const updated={...t,colaboradorId:sg.id,estado:"Presupuestando",historial:[...getHistorial(t),{ts:now(),txt:`Asignado a ${sg.nombre}`,tipo:"sistema"},{ts:now(),txt:"Presupuesto solicitado WA",tipo:"wa"}]};
            await dbSaveTrabajo(updated);
            setData(d=>({...d,trabajos:d.trabajos.map(x=>x.id===t.id?{...x,colaboradorId:sg.id,colaborador_id:sg.id,estado:"Presupuestando"}:x)}));
            toast("📱 Abriendo WhatsApp...");
            setTimeout(()=>window.open(waUrl,"_blank"),400);
          }} className="bg-orange-500 hover:bg-orange-600 text-white text-[11px] font-bold px-3 py-1.5 rounded-lg transition whitespace-nowrap">Asignar WA</button>}
        </div>;
      })}
    ...
    {proximas.length>0&&<div>
      ...
      <div className="space-y-2">{proximas.map(t=>{
        const cl=data.clientes.find(c=>c.id===getClienteId(t));
        const co=data.colaboradores.find(c=>c.id===getColabId(t));
        return<div key={t.id} onClick={()=>setTid(t.id)} className="bg-white border border-gray-100 rounded-xl px-4 py-3 shadow-sm flex items-center gap-3 cursor-pointer hover:border-[#1E3A5F] transition">
          <div className="text-center min-w-[36px]"><div className="text-xl font-black text-[#1E3A5F] leading-none">{new Date(t.fecha+"T00:00:00").getDate()}</div><div className="text-[9px] text-gray-400 uppercase">{new Date(t.fecha+"T00:00:00").toLocaleDateString("es-ES",{month:"short"})}</div></div>
          <div className="flex-1 min-w-0"><div className="font-semibold text-sm text-gray-800 truncate">{t.tipo} — {cl?.nombre}</div><div className="text-xs text-gray-400">{t.hora} · {co?.nombre||"Sin asignar"}</div></div>
          <Badge text={t.estado}/>
        </div>;
      })}</div>
    </div>}
        ...
        {sg?<div className="bg-orange-50 border border-orange-200 rounded-xl p-3" onClick={e=>e.stopPropagation()}>
          ...
          <div className="flex gap-2">
            <button onClick={async(e)=>{e.stopPropagation();
              ...
              await dbSaveTrabajo(updated);
              setData(d=>({...d,trabajos:d.trabajos.map(x=>x.id===t.id?{...x,colaboradorId:sg.id,colaborador_id:sg.id,estado:"Presupuestando"}:x)}));
              toast("📱 Abriendo WhatsApp...");setTimeout(()=>window.open(waUrl,"_blank"),400);
            }} className="flex-1 bg-orange-500 hover:bg-orange-600 text-white text-sm font-bold py-2 rounded-xl transition">Asignar y pedir presupuesto 📱</button>
...
function EstadoDemandas({data,setData,onBack,toast,onVer}){
  ...
  const GRUPOS=[
    ...
    {estado:"Visita confirmada",color:"border-teal-200 bg-teal-50",dot:"bg-teal-500",label:"✅ Visita confirmada — avisar colaborador"},
  ...
  return<div>
    ...
    <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm mb-4">
      <div className="flex gap-0.5 h-3 rounded-full overflow-hidden mb-3">
        {[{e:"Solicitud",bg:"bg-red-400"},{e:"Presupuestando",bg:"bg-amber-400"},{e:"Visita confirmada",bg:"bg-teal-400"},{e:"Presupuesto recibido",bg:"bg-purple-500"},{e:"En curso",bg:"bg-orange-500"},{e:"Completado",bg:"bg-emerald-500"}].map(({e,bg})=>{
          const n=data.trabajos.filter(t=>t.estado===e).length;
          return<div key={e} className={`${bg} transition-all`} style={{width:`${(n/total)*100}%`,minWidth:n>0?"6px":"0"}}/>;
        })}
      </div>
      ...
      {grupos.map(g=>{
        ...
        return<div key={g.estado}>
          ...
          <div className="space-y-2">{its.map(t=>{
            ...
            return<div key={t.id} className={`border rounded-2xl p-4 shadow-sm ${g.color}`}>
              ...
              <div className="flex gap-2 mt-3">
                ...
                {accion&&<button onClick={async()=>{
                  ...
                  if(!["Completado","Cancelado"].includes(t.estado)){
                    const next=FLUJO[idx+1];
                    const updated={...t,estado:next,historial:[...getHistorial(t),{ts:now(),txt:`Estado → ${next}`,tipo:"sistema"}]};
                    await dbSaveTrabajo(updated);
                    setData(d=>({...d,trabajos:d.trabajos.map(x=>x.id===t.id?{...x,estado:next}:x)}));
                    toast(`→ ${next}`);
                  }
...
function EstadoDemandas({data,setData,onBack,toast,onVer}){
  ...
  if(busca.trim()){const q=busca.toLowerCase();items=items.filter(t=>{const cl=data.clientes.find(c=>c.id===getClienteId(t));return t.descripcion?.toLowerCase().includes(q)||cl?.nombre.toLowerCase().includes(q);});}
  items.sort((a,b)=>{const o={"Solicitud":0,"Presupuestando":1,"Presupuesto enviado":2,"Aceptado":3,"En curso":4,"Completado":5,"Cancelado":6};return(o[a.estado]||0)-(o[b.estado]||0);});
  const total=data.trabajos.filter(t=>t.estado!=="Cancelado").length||1;
  return<div>
    <Back title="Estado de demandas" onBack={onBack}/>
    <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm mb-4">
      <div className="flex gap-0.5 h-3 rounded-full overflow-hidden mb-3">
        {[{e:"Solicitud",bg:"bg-slate-300"},{e:"Presupuestando",bg:"bg-amber-400"},{e:"Presupuesto enviado",bg:"bg-blue-500"},{e:"Aceptado",bg:"bg-violet-500"},{e:"En curso",bg:"bg-orange-500"},{e:"Completado",bg:"bg-emerald-500"}].map(({e,bg})=>{
          const n=data.trabajos.filter(t=>t.estado===e).length;
          return<div key={e} className={`${bg} transition-all`} style={{width:`${(n/total)*100}%`,minWidth:n>0?"5px":"0"}}/>;
        })}
      </div>
      <div className="grid grid-cols-6 gap-1 text-center">
        {["Solicitud","Presupuestando","Presupuesto enviado","Aceptado","En curso","Completado"].map(e=><div key={e}><div className="font-black text-gray-800">{data.trabajos.filter(t=>t.estado===e).length}</div><div className="text-[8px] text-gray-400 leading-tight">{e.split(" ")[0]}</div></div>)}
      </div>
    </div>
    <input className={S+" mb-3"} placeholder="🔍 Buscar..." value={busca} onChange={e=>setBusca(e.target.value)}/>
    <div className="mb-3"><div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Estado</div><div className="flex gap-1.5 flex-wrap">{["Todos",...FLUJO.filter(e=>e!=="Cancelado")].map(e=><Pill key={e} label={e} active={fEstado===e} onClick={()=>setFEstado(e)}/>)}</div></div>
    <div className="mb-3"><div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Servicio</div><div className="flex gap-1.5 flex-wrap">{["Todos",...TIPOS].map(t=><Pill key={t} label={t} active={fTipo===t} onClick={()=>setFTipo(t)}/>)}</div></div>
    <div className="mb-4"><div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Canal</div><div className="flex gap-1.5 flex-wrap">{["Todos",...ORIGENES.map(o=>o.id)].map(id=>{const o=id==="Todos"?{label:"Todos",icon:""}:orCfg(id);return<Pill key={id} label={`${o.icon?o.icon+" ":""}${o.label}`} active={fOrigen===id} onClick={()=>setFOrigen(id)}/>;})}</div></div>
    ...
    <div className="space-y-2">{items.map(t=>{
      ...
      return<div key={t.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-1.5 mb-1"><Badge text={t.estado}/><OrigenTag id={t.origen}/><span className={`text-[10px] font-bold ${PRIO_CFG[t.prioridad]?.text}`}>{PRIO_CFG[t.prioridad]?.icon}</span></div>
          ...
          <div className="text-right flex-shrink-0">{getPrecioCliente(t)&&<div className="text-sm font-black text-emerald-700">{getPrecioCliente(t)}€</div>}{getPresupColab(t)&&getPrecioCliente(t)&&<div className="text-[10px] text-gray-400">colab: {getPresupColab(t)}€</div>}</div>
        </div>
        <div className="flex gap-2">
          ...
          {co&&["Presupuestando","Aceptado"].includes(t.estado)&&<button onClick={()=>{window.open(buildWA(co,t,cl),"_blank");toast("📱 WhatsApp...");}} className="bg-green-500 text-white text-xs font-bold px-3 py-2 rounded-xl hover:bg-green-600 transition">📱</button>}
          {!["Completado","Cancelado"].includes(t.estado)&&<button onClick={async()=>{
            const next=FLUJO[idx+1];
            const updated={...t,estado:next,historial:[...getHistorial(t),{ts:now(),txt:`Estado → ${next}`,tipo:"sistema"}]};
            await dbSaveTrabajo(updated);
            setData(d=>({...d,trabajos:d.trabajos.map(x=>x.id===t.id?{...x,estado:next}:x)}));
...
function Clientes({data,setData,onBack,toast}){
  ...
  if(!cid){
    ...
    return<div>
      ...
      <div className="space-y-2">{list.map(c=>{
        ...
        return<div key={c.id} onClick={()=>setCid(c.id)} className="bg-white border border-gray-100 rounded-2xl px-4 py-4 shadow-sm cursor-pointer hover:border-[#1E3A5F] hover:shadow-md transition">
          <div className="flex items-center justify-between mb-1"><div className="font-bold text-gray-800">{c.nombre}</div><span className="text-[#1E3A5F] text-xl">›</span></div>
      ...
      {showNew&&<Modal title="Nuevo cliente" onClose={()=>setShowNew(false)}>
        ...
        return<><Fld label="Nombre *"><input className={S} value={f.nombre} onChange={e=>setF(x=>({...x,nombre:e.target.value}))}/></Fld>
        ...
        <button onClick={async()=>{if(!f.nombre.trim())return;const saved=await dbSaveCliente({...f,creado:hoy()});if(saved){setData(d=>({...d,clientes:[...d.clientes,saved]}));setShowNew(false);toast("✅ Cliente creado");}}} className="w-full bg-[#1E3A5F] hover:bg-[#152d4a] text-white py-2.5 rounded-xl font-bold text-sm transition">Guardar</button></>;})()}
      </Modal>}
    </div>;
  }
  ...
  return<div>
    ...
    <div className="space-y-2">{hist.map(t=><div key={t.id} className="bg-white border border-gray-100 rounded-2xl p-3 shadow-sm">
      <div className="flex items-start justify-between gap-2 mb-1"><div><div className="font-semibold text-sm text-gray-800">{t.tipo}</div><div className="text-xs text-gray-500 truncate">{t.descripcion}</div></div><Badge text={t.estado}/></div>
      <div className="flex items-center gap-2"><OrigenTag id={t.origen}/><span className="text-[10px] text-gray-400">📅 {fmt(t.fecha)}</span>{getPrecioCliente(t)&&<span className="text-xs font-bold text-emerald-700 ml-auto">{getPrecioCliente(t)}€</span>}</div>
    </div>)}</div>
  </div>;
}
  ...
  if(!coid){
    ...
    return<div>
      ...
      <div className="space-y-2">{list.map(c=>{
        ...
        return<div key={c.id} onClick={()=>setCoid(c.id)} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm cursor-pointer hover:border-[#1E3A5F] hover:shadow-md transition">
          <div className="flex items-start justify-between mb-2">
            <div><div className="flex items-center gap-2 flex-wrap"><span className="font-bold text-gray-800">{c.nombre}</span><span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${c.activo?"bg-emerald-100 text-emerald-700":"bg-gray-100 text-gray-400"}`}>{c.activo?"Activo":"Inactivo"}</span>{act>0&&<span className="text-[10px] bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full font-bold">{act} en curso</span>}</div><div className="text-xs text-gray-500 mt-0.5">{c.especialidades?.join(" · ")}</div><div className="text-xs text-gray-400">{c.telefono}{c.zona?` · ${c.zona}`:""}</div></div>
            <div className="text-right"><span className="text-[#1E3A5F] text-xl">›</span>{pag>0&&<div className="text-xs text-red-500 font-bold">{pag}€</div>}</div>
          </div>
          <div className="flex gap-0.5">{DIAS.map((d,i)=><span key={i} className={`text-[9px] w-6 h-6 flex items-center justify-center rounded-lg font-bold ${c.disponibilidad?.includes(i)?"bg-emerald-100 text-emerald-600":"bg-gray-100 text-gray-300"}`}>{d}</span>)}</div>
        </div>;
      })}
